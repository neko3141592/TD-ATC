...
[ExecuteAlways]
public class PressureGaugeScaleGenerator : MonoBehaviour
{
    [Header("Target")]
    ...
    [SerializeField, Min(0.1f)] private float majorStepKPa = 50f;
    [SerializeField, Min(0.1f)] private float minorStepKPa = 10f;
...
#if UNITY_EDITOR
        // Unityの初期化途中で重複生成しないために予約を入れ替える
    ...
    public void GenerateScale()
    {
        ...
        float safeMinorStep = Mathf.Max(0.1f, minorStepKPa);
        float safeMajorStep = Mathf.Max(safeMinorStep, majorStepKPa);
        float pressureRange = Mathf.Max(0.001f, maxPressureKPa - minPressureKPa);
        int guard = 0;

        for (float pressure = minPressureKPa; pressure <= maxPressureKPa + 0.001f; pressure += safeMinorStep)
        {
            guard++;
            if (guard > 2000)
            {
            ...
            bool isMajor = IsMultipleOfStep(pressure - minPressureKPa, safeMajorStep);
            float t = Mathf.Clamp01((pressure - minPressureKPa) / pressureRange);
            if (invertDirection)
            {
                t = 1f - t;
            }
            ...
            CreateTick(pressure, angle, tickLength, tickWidth);

            if (isMajor && generateLabels)
            {
                CreateLabel(pressure, angle);
            }
        }
    }
    ...
    private void CreateLabel(float pressureKPa, float angle)
    {
        ...
        text.text = $"{Mathf.RoundToInt(pressureKPa)}{labelSuffix}";
